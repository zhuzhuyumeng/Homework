...
def LFM(train, ratio, K, lr, step, lmbda, N):
    ...
    def nSample(data, ratio):
        ...
        for user in new_data:
            ...
            pos_num = len(seen)
            # 乘3增加候选项目的多样性,这里的item被用到了下面的遍历负样本中
            item = np.random.choice(items, int(pos_num * ratio * 3), pops)
            # 只保留前(pos_num*ratio)个负样本
...
class Experiment():
    ...
    def __init__(self, M, N, ratio=1,
                 K=100, lr=0.02, step=100, lmbda=0.01, fp='F:/data/ml-100k/u.data'):
        '''
        :params: M, 进行多少次实验
    ...
    def run(self):
        ...
        for ii in range(self.M):
            ...
            metric,user_predict = self.worker(train, test)
...
M, N = 2, 10
exp = Experiment(M, N, ratio=1)
user_predict = exp.run()
# 初始化变量
user_input = None
...
while user_input != 0:
    ...
    # 输出输入的数字（如果不是0）
    if user_input != 0:
        print(f"您选择的用户是：{user_input}")
        print(user_predict[user_input])
